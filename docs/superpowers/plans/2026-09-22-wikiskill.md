...
```
Budget, in calls and tokens (the runs use the claude.ai subscription, so `total_cost_usd` is an API-equivalent estimate, not a charge; the real constraint is usage limits). Measured on the full run, Haiku 4.5: ≈ 9k output tokens per LiveMath inference call (8k without skills, up to 15k with a skill injected), ~60–90 s each. Calls: 124 no-skill test + 18 baseline val + 8×(35 train + 18 val + Maintainer + Proposer) + 124 WikiSkill test ≈ 710 calls ≈ 6.5M output tokens. `engram evolve status` shows calls / in_tok / out_tok / API minutes per iteration and in total. Record the two test numbers and `status` in `PLAN.md`.
