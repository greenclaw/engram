...
```
Budget, in calls and tokens (the runs use the claude.ai subscription, so `total_cost_usd` is an API-equivalent estimate, not a charge; the real constraint is usage limits). Measured on the full run, Haiku 4.5, per inference call (medians): ~7.5k output tokens; input 1.2k without skills, 5.5k with the evolved 270-line skill; ~70–80 s. Maintainer/Proposer calls ~115k input. Calls: 124 no-skill test + 18 baseline val + 8×(35 train + 18 val + Maintainer + Proposer) + 124 WikiSkill test = 706 calls, 4.1M input / 6.7M output tokens, ~3 h wall-clock at `--parallel 8`. `engram evolve status` shows calls / in_tok / out_tok / API minutes per iteration and in total. Record the two test numbers and `status` in `PLAN.md`.
