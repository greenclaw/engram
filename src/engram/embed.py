...
_REPO = os.environ.get("ENGRAM_EMBED_REPO", "BAAI/bge-m3")
...
def _resolve() -> tuple[str, str]:
    ...
    onnx = hf_hub_download(_REPO, "onnx/model.onnx", local_files_only=True)
    hf_hub_download(_REPO, "onnx/model.onnx_data", local_files_only=True)  # external weights, co-located
    tok = hf_hub_download(_REPO, "tokenizer.json", local_files_only=True)
    return onnx, tok
