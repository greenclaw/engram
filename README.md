...
- **Increment 2 (curator): deterministic core BUILT** (2026-07-02) — `engram curate apply`: change-set → unified diff → human gate → git commit (path-contained, pathspec-only commits). **The novel half — LLM adjudication (skill) — and the `mem_curate` bench are NOT built yet**; until they land, the curator thesis is unvalidated.
- **Next:** `/engram-curate` skill (Claude adjudicates → emits change-set) → live recall wiring → `mem_curate` bench. See `PLAN.md`.
