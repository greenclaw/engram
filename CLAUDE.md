...
- `uv run engram evolve init --ws <dir> --bench livemath|spreadsheetbench --seed 0` / `run --ws <dir> --model haiku --iters 8` / `eval --ws <dir> --split test [--skills <other-ws>]` / `status --ws <dir>` — **WikiSkill** (Tang et al., arXiv:2608.27454) skill evolution, Algorithm 1 as a deterministic harness: three-layer workspace (`raw/ wiki/ skills/`, its own git repo, not the memory store), Inference Agent / Wiki Maintainer / ReAct Skill Proposer as headless `claude -p` with the Appendix E prompts (`src/engram/wikiskill/prompts/`), strict validation gate (Eq. 4) with skills-only rollback via `accepted-<k>` tags, `wiki/skill-impact.md` written by the harness only. Failed `claude` calls fail the rollout loud (`<id>.error.json`, re-run retries only those); `run` resumes mid-iteration without re-running the Maintainer or Proposer (`state.json` `pending`); `eval` traces are cached per skill content. LiveMath caveat: the meta option "…a stronger result can be proven" is the gold answer on 46% of items and never a distractor — a shortcut the Proposer finds (spec §Known benchmark property). Consumption is reported in calls / tokens / API minutes, never USD (`claude -p` runs on the subscription; `total_cost_usd` is only an API-equivalent): measured on the full LiveMath run with Haiku, per inference call (medians): ~7.5k output tokens, input 1.2k without skills and 5.5k with the evolved 270-line skill injected, ~70–80 s; Maintainer/Proposer calls ~115k input; `status` shows per-iteration and total usage. Spec: `docs/superpowers/specs/2026-09-22-wikiskill-design.md`. **SpreadsheetBench** (Verified 400, 80/40/280): the agent runs Python in a Claude Code bash sandbox (denyRead ~/; allowRead = task workdir + agent venv + the whole uv interpreter root, or the venv's python3 falls through to /usr/bin/python3); graded by a port of the official `compare_workbooks` (0 disagreements on 800 comparisons, ceiling 396/400); the Proposer's Read is denied val/eval traces, splits and golden data. Spec: `docs/superpowers/specs/2026-09-23-wikiskill-spreadsheetbench-design.md`.
